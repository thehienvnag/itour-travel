 ...
 * @author thehien
 */
@WebServlet(name = "FinishBookingServlet", urlPatterns = {"/FinishBookingServlet"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (bookingCode != null) {
                ...
                BookingDTO booking = bookingDAO.getBookingByCode(bookingCode);
                HttpSession session = request.getSession();
                RegistrationDTO user = (RegistrationDTO) session.getAttribute(Config.ATTR_USER);
                if (user.getRoleId() == Config.ROLE_ADMIN || user.getId() == booking.getUserId()) {
                    request.setAttribute(Config.ATTR_BOOKING, booking);
                    request.setAttribute(Config.ATTR_PAYMENT_TYPE, "Charge with momo");
                } else {
                    url = Config.JSP_HOME;
                }
            ...
            Logger.log(ex.getMessage(), FinishBookingServlet.class);
