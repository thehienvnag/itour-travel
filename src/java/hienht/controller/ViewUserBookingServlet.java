 ...
 * @author thehien
 */
@WebServlet(name = "ViewUserBookingServlet", urlPatterns = {"/ViewUserBookingServlet"})
public class ViewUserBookingServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = Config.JSP_BOOKING_HISTORY;
        try  {
            HttpSession session = request.getSession();
            RegistrationDTO userInfo = (RegistrationDTO) session.getAttribute(Config.ATTR_USER);
            
            BookingDAO bookingDAO = new BookingDAO();
            List<BookingDTO> listBooking = bookingDAO.getBookingByUserId(userInfo.getId());
            
            request.setAttribute(Config.ATTR_BOOKING_LIST, listBooking);
        } catch (SQLException ex) {
            Logger.log(ex.getMessage(), ViewUserBookingServlet.class);
